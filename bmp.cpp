...
bmp::bmp(const char* fileName, const int width, const int height, PixelInfo* getPix) : file(std::ofstream(fileName, std::ios::binary | std::ios::out)), width(width), height(height) {
	pixelInfo = getPix;
